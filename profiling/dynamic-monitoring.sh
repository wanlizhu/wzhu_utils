#!/usr/bin/env bash
#
# dynamic-monitoring.sh - Sample CPU and GPU metrics for a target process and write a merged CSV (and optional HTML chart).
#
# WHAT THIS SCRIPT DOES
#   While a target process is running (or until you press Ctrl-C or a time limit is reached), the script:
#   1. Runs two background samplers in separate process groups:
#      - CPU sampler: periodically reads /proc/stat, /proc/meminfo, and /proc/<pid>/stat to record wall-clock
#        time, system-wide CPU utilization, host memory/swap usage, and the target process's CPU share and RSS.
#      - GPU sampler: runs nvidia-smi --loop-ms to record GPU metrics (P-state, utilization, clocks, power,
#        temperature, memory, etc.) with the same interval.
#   2. Writes raw samples to temporary CSV files (TMP_CPU_FILE, TMP_GPU_FILE).
#   3. After the target exits or monitoring stops, merges the two CSVs into one time-aligned CSV (OUT_CSV).
#   4. Optionally invokes draw-polylines-in-html.py on the merged CSV to generate an interactive HTML chart.
#
# IMPLEMENTATION IN DETAIL
#   - Target selection: You pass either a PID or the word "steam". For "steam", the script prints the Steam
#     process tree and prompts for the game PID. The main loop and CPU sampler both check that /proc/<pid>
#     exists; the CPU sampler exits when the target disappears.
#
#   - Sampling interval and duration: DM_SAMPLE_FREQ_MS (default 100 ms) defines the period for both
#     samplers. DM_SAMPLE_TIME_LIMIT_S (default 0) optionally stops monitoring after that many seconds;
#     otherwise monitoring runs until Ctrl-C or target exit.
#
#   - Timestamps: Both samplers use a synthetic timestamp base_ts_ms + sample_idx * interval_ms so rows are
#     strictly increasing and avoid duplicate seconds from date +%s%3N. The merge step aligns the CPU stream
#     to the first GPU timestamp by picking the CPU row whose time is closest to the first GPU sample, then
#     emits rows in GPU time order; if any merged timestamp is not strictly increasing, it is bumped by 1 ms.
#
#   - CPU sampler (start_cpu_sampler): Launched with setsid so it can be stopped as a process group. Reads
#     /proc/stat twice per sample to compute system CPU % from (total_delta - idle_delta) / total_delta;
#     reads /proc/<pid>/stat twice to compute target CPU % as target_jiffy_delta / total_delta; converts
#     target RSS from pages to MiB using getconf PAGESIZE. Output columns: ts_ms, cpu_util_pct, cpu_mem_used_mb,
#     swap_used_mb, target_cpu_pct, target_rss_mb.
#
#   - GPU sampler (start_gpu_sampler): Launched with setsid. Uses GPU_QUERY_FIELDS (with optional :unit
#     suffixes) to build the CSV header via build_gpu_csv_header and the nvidia-smi query string via
#     build_nvidia_smi_query_fields (units stripped). For each nvidia-smi line, convert_gpu_value() normalizes
#     pstate (P0/P1/... -> 0/1/...) and clocks_event_reasons.active (hex -> decimal); other fields are
#     passed through. Header is ts_ms plus one column per field in GPU_QUERY_FIELDS (names normalized by
#     normalize_gpu_field_name: dots/colons replaced with underscores, suffix _<unit>).
#
#   - Merge (merge_cpu_gpu_csv): AWK reads both files; aligns CPU to GPU start; prints one header line
#     (GPU header + CPU columns without leading ts_ms) and then paired data rows in GPU order, length limited
#     by the shorter stream. Ensures strictly increasing first column for downstream tools (e.g. draw-polylines-in-html.py).
#
#   - Cleanup and signals: trap cleanup EXIT stops both process groups on exit. trap request_stop INT TERM
#     sets stop_requested=1 so the main loop exits gracefully; cleanup then stops the samplers and the merge
#     still runs if temp files exist. Samplers are stopped by killing the process group (kill -- -pid).
#
#   - Post-processing: If draw-polylines-in-html.py is in PATH, it is run with OUT_CSV so the merged data
#     is turned into an interactive HTML chart (same stem as OUT_CSV, .html).
#
# REQUIREMENTS
#   Bash, nvidia-smi, /proc (Linux). Optional: draw-polylines-in-html.py for HTML output.
#
set -o pipefail
