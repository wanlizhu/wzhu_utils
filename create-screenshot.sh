    ...
    if [[ $(list-login-session.sh seat0.type) == wayland ]]; then 
        if [[ $XDG_CURRENT_DESKTOP == *GNOME* ]]; then
            ...
            timeout 5 gnome-screenshot -f $output || {
                fallback_nvidia_fbc=true 
                echo "Press Alt+F2, enter lg, set unsafe-mode flag to use gnome-screenshot"
            }
        else 
            [[ -z $(which grim) ]] && sudo apt install -y grim &>/dev/null 
            timeout 5 grim $output || fallback_nvidia_fbc=true 
        fi  
    else
        [[ -z $(which magick) && -z $(which import) ]] && sudo apt install -y imagemagick
        if command -v magick > /dev/null; then
            timeout 5 magick import -window root $output || fallback_nvidia_fbc=true 
        elif command -v import > /dev/null; then
            timeout 5 import -window root $output || fallback_nvidia_fbc=true  
        fi
    fi 
fi 
